<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.xiaoshuoName" type="text" placeholder="作品"></input>
		</view>
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.xiaoshuoName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.xiaoshuoPhoto?product.xiaoshuoPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.xiaoshuoNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.xiaoshuoNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('xiaoshuoCollection','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('xiaoshuoCollection','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('xiaoshuoCollection','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let xiaoshuoCollectionTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'xiaoshuo_collection_types'
		  	});
		  	this.xiaoshuoCollectionTypesList = xiaoshuoCollectionTypes.data.list;
            //当前表的 作品类型 字段 字典表查询方法
		  	let xiaoshuoTypes = await this.$api.page('dictionary', {
                page: 1,
                limit: 100,
                dicCode:'xiaoshuo_types'
            });
		  	this.xiaoshuoTypesList = xiaoshuoTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.xiaoshuo_collectionName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`xiaoshuoCollection`, params);
			...
			onDetailTap(item) {
				this.$utils.jump(`../xiaoshuo/detail?id=${item.xiaoshuoId}`)
			},
			// 修改
			onUpdateTap(id) {
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaoshuoCollection', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.xiaoshuoName) {
					searchForm['xiaoshuoName'] = this.searchForm.xiaoshuoName
				}

                let res = await this.$api.page(`xiaoshuoCollection`, searchForm);
				// 如果是第一页数据置空
