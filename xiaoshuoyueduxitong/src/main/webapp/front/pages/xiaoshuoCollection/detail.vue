   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#A27DE3","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.xiaoshuoCollectionValue}}</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('xiaoshuoCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.xiaoshuoCollectionPhoto ? this.detail.xiaoshuoCollectionPhoto.split(",") : [];
			},
