<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.xiaoshuoLiuyanText" type="text" placeholder="评论内容"></input>
		</view>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;width: 800rpx;">
			<view style="font-size: 30rpx;">
				<b>{{item.xiaoshuoName}}</b>
			</view>
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">我的评论：{{item.xiaoshuoLiuyanText}}</text>
			</view>
        ...
        <view style="float: right;margin-right: 25upx;">
            <text v-if="isAuth('xiaoshuoLiuyan','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('xiaoshuoLiuyan','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.xiaoshuo_liuyanName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`xiaoshuoLiuyan`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaoshuoLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.xiaoshuoLiuyanText) {
                    searchForm['xiaoshuoLiuyanText'] = this.searchForm.xiaoshuoLiuyanText
                }

                let res = await this.$api.page(`xiaoshuoLiuyan`, searchForm);
				// 如果是第一页数据置空
