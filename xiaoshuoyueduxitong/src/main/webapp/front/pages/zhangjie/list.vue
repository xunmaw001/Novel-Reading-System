	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.zhangjieName" type="text" placeholder="章节标题"></input>
			</view>
			<view class="action">
				<button @tap="search"
		...
		<view class="list b-b" v-for="(item, index) in list" :key="index">
			<view  @tap="onDetailTap(item)" class="wrapper" style="margin-left: 25upx;margin-top: 10upx;">
				<view style="font-size: 30rpx;">
					<b>{{item.zhangjieName}}</b>
				</view>
				<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
					<text style="margin-left: 30rpx;">章节内容：
						<text v-if='item.zhangjieContent.length>10'>{{item.zhangjieContent.slice(0,10)}}....</text>
						<text v-else>{{item.zhangjieContent}}</text>
					</text>
				</view>
			</view>
		...
		<button
			...
			v-if="isAuth('zhangjie','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (uni.getStorageSync("xiaoshuoId") != null) {
					params.xiaoshuoId = uni.getStorageSync("xiaoshuoId")
				}

				let res = await this.$api.page(`zhangjie`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhangjie', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.zhangjieName) {
					searchForm['zhangjieName'] = this.searchForm.zhangjieName
				}
				if (this.searchForm.zhangjieName) {
					searchForm['zhangjieName'] = this.searchForm.zhangjieName
				}

				let res = await this.$api.page(`zhangjie`, searchForm);
				// 如果是第一页数据置空
