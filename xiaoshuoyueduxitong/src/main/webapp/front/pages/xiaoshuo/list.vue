	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.xiaoshuoName" type="text" placeholder="作品名称"></input>
			</view>
		...
		<scroll-view scroll-x="true" class="tabView"
			...
			<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
				v-for="(item,index) in xiaoshuoTypesList" :key="index"
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
			<view @tap="onDetailTap(product)" class="uni-product"
				:style='{"borderRadius":"50rpx","backgroundColor":"#A27DE3"}' v-for="(product,index) in list"
				:key="index">
				<view class="uni-product-title"
					:style='{"fontSize":"26rpx","lineHeight":"40rpx","color":"#fff","textAlign":"center"}'>
					{{product.xiaoshuoName}}
				</view>
				<view class="image-view">
					<image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
						class="uni-product-image" :src="product.xiaoshuoPhoto?product.xiaoshuoPhoto.split(',')[0]:''">
					</image>
				</view>
				...
				<view style="display: flex;justify-content: space-between;">
					<text v-if="isAuth('xiaoshuo','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('xiaoshuo','删除')" class="cuIcon-delete"
			...
			v-if="isAuth('xiaoshuo','新增')" class="add-btn" @click="onAddTap()">新增</button>
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != 0) {
					params.xiaoshuoTypes = this.categoryName
				}

				let res = await this.$api.list(`xiaoshuo`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaoshuo', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				let res = await this.$api.list(`xiaoshuo`, searchForm);
				// 如果是第一页数据置空
