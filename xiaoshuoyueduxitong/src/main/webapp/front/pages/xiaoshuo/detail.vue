	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<!-- 名字-->
					<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.xiaoshuoName}}
					</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.xiaoshuoContent"></rich-text>
				</view>
			</view>
		...
		<view class="bottom-content cu-bar bg-white tabbar border shop">
			...
			<view style="text-align: right;">
				<button v-if="shujiaList<=0" @tap="onSubmit" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
                            :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">立即加入书架</button>
				<button @tap="chakanzhuangjie" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
			            :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">查看章节</button>
			</view>
		</view>
		</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			if(this.shujiaList <= 0){
				let res = await this.$api.page("shujia",{
					page:1,
					limit:999,
					xiaoshuoId:this.id
				});
				this.shujiaList = res.data.list.length
			}
		...
		methods: {
			async onSubmit(){
				 await this.$api.save(`shujia`, {
					yonghuId:this.user.id,
					xiaoshuoId:this.id
				 });
				this.shujiaList = 2
				this.$utils.msg('已成功加入书架');
			},
			async chakanzhuangjie(){
				let _this = this
				uni.removeStorageSync("xiaoshuoId")
				uni.setStorageSync("xiaoshuoId",_this.detail.id);
				this.$utils.jump(`../zhangjie/list`)
			},
			//下载
			download(url) {
				let _this = this;
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('xiaoshuo', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.xiaoshuoPhoto ? this.detail.xiaoshuoPhoto.split(",") : [];
			},
			// 收藏
			async getxiaoshuoCollection() {
				let params = {
					page: 1,
					limit: 1,
					xiaoshuoId: this.id,
				...
				let res = await this.$api.list(`xiaoshuoCollection`, params);
			...
			async shoucang() {
				...
				let res = await _this.$api.list(`xiaoshuoCollection`, params);
				if (res.data.list.length == 1) {
					let xiaoshuoCollectionId = res.data.list[0].id;
					uni.showModal({
						title: '提示',
						content: '是否取消',
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('xiaoshuoCollection', JSON.stringify([
									xiaoshuoCollectionId
								]));
								_this.$utils.msg('取消成功');
								_this.getxiaoshuoCollection();
							}
						}
					});
					return;
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('xiaoshuoCollection', {
								yonghuId: _this.user.id,
								xiaoshuoId: _this.detail.id,
								xiaoshuoCollectionTypes: 1
							});
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('xiaoshuoLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					xiaoshuoId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../xiaoshuoLiuyan/add-or-update?xiaoshuoId=${this.id}`)
			},
		}
	}
</script>
